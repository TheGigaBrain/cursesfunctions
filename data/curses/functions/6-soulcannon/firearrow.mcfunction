...
execute as @s[scores={soultimer=57}] at @s run execute at @p[distance=..32,gamemode=!creative,gamemode=!spectator] anchored feet positioned ~ ~-1 ~ run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:5,Tags:["soultarget"]}

# The projectiles to be used
# We check for a player to prevent entity spam in certain edge cases
execute as @s[scores={soultimer=60}] at @s if entity @p[distance=..32,gamemode=!creative,gamemode=!spectator] facing entity @e[tag=soultarget,sort=nearest,limit=1] feet rotated ~ 0 run summon minecraft:arrow ^ ^1.5 ^0.5 {damage:4,pickup:0,crit:1,Fire:1200,CustomPotionEffects:[{Id:20,Amplifier:0,Duration:200}],Tags:["soulcannon"],SoundEvent:"minecraft:entity.generic.explode"}
execute as @s[scores={soultimer=60}] at @s if entity @p[distance=..32,gamemode=!creative,gamemode=!spectator] facing entity @e[tag=soultarget,sort=nearest,limit=1] feet rotated ~ 0 run summon minecraft:area_effect_cloud ^ ^1.5 ^0.5 {Duration:200,WaitTime:20,DurationOnUse:0,RadiusOnUse:0,RadiusPerTick:0,Radius:4,Particle:"minecraft:dust 0.25 0 0.25 1.5",Effects:[{Id:20,Amplifier:4,Duration:10,ShowParticles:0}],Tags:["movingsoulfield","soulfield"]}
...
execute as @s[scores={soultimer=60}] at @s if entity @p[distance=..32,gamemode=!creative,gamemode=!spectator] run playsound entity.blaze.shoot hostile @a ~ ~ ~ 1 2
